	...
	$.ajax({
		...
		success: function (data) {
			...
			$.each(data, function(index, element) {
	    			...
	    			"</div>" +
	    			"<a href='../php/add_to_wishlist.php?event_id="+element.event_id+"' class='btn btn-info' style='margin:10px 100px 10px 100px;text-transform: uppercase;text-align:center;' role='button' id='"+element.event_id+"'>Add to Wishlist</a>" +
	    			"</div>" +
	    			"</div>"; 
